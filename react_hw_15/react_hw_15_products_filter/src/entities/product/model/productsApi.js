...
export const productsApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getProducts: builder.query({
      async queryFn({ page = 1, perPage = 6, cursors = [], searchTerm  = ''}) {
        try {
          const { data, cursor, hasMore } = await db.getAllPaginatedWithFilter({
            page,
            perPage,
            cursors,
				searchTerm,
    ...
    getProductById: builder.query({
      async queryFn(id) {
			if (!id) {
      return { error: { status: 400, message: 'Product ID is required' } }
    }
