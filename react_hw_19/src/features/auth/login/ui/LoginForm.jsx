...
export default function LoginForm() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMessage(err?.message || t('loginForm.ERROR'))
    }
  }
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      setErrorMessage(err?.message || t('loginForm.GOOGLE_ERROR'))
    }
  }
    ...
    <form onSubmit={onSubmit} className="w-full flex flex-col gap-4">
      ...
      {(error || errorMessage) && (
        <div className="text-red-500 dark:text-red-400 text-sm font-medium mt-1">
          {errorMessage || error?.data?.message || t('loginForm.GOOGLE_ERROR')}
        </div>
      )}
    </form>
  )
